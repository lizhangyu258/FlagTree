...
@dialect(name="cuda", file=Path(__file__).parent / "04-tle-raw-pipeline-producer.cu")
...
def matmul(a, b, activation=""):
    ...
    K, N = b.shape
    if min(M, N, K) <= 0:
        raise ValueError("LoadTiles requires nonempty matrices")
    # Allocates output.
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    a = torch.rand((1024, 1024), device=DEVICE, dtype=torch.float16) - 0.5
    b = torch.rand((1024, 1024), device=DEVICE, dtype=torch.float16) - 0.5
